...
- **Cohesive UI & Test Dispatches:** When dispatching UI-related changes (such as adding or removing buttons, inputs, or modifying layouts), always specify both the UI file path and its associated test file path in the same print-mode prompt. This guides the coding engine to modify both the component and its tests in a single, aligned turn, avoiding disjoint commits or immediate test suites breakages.
- **Shell Backtick and `${var}` Expansion Trap:** When calling Claude Code (`claude -p`) or any CLI tool via the terminal tool within double-quoted command strings, avoid using unescaped backticks (`` `...` ``) or dollar-braces (like `${tournament.id}`) in your prompt.
  1. The host shell interprets backticks as active command substitutions (throwing errors like `/bin/bash: command not found`).
  2. The host shell interprets `${...}` as variable expansion (throwing `/bin/bash: bad substitution` due to dots, or silently expanding to empty strings).
  To bypass this trap completely, **always wrap the prompt string in single quotes** (`claude -p '...'`). If the prompt contains single quotes (e.g., `'picks_locked'`), close the single quote, escape a single quote, and open it again: `status === '\''picks_locked'\''`. Never use double quotes for the outer prompt string if it contains any `$`, `` ` ``, or backslashes.
