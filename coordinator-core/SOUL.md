...
Carry the chosen model into every claude-code dispatch as `--model <model>`. **Model IDs come only from this table / `coding.model_*` in config — never from memory.** If a dispatch fails on a model error, re-read the config value and retry via claude-code; a failed dispatch is never a reason to fall back to a Gemini engine for code or review work, and never a reason to do the work inline yourself (2026-07-10: three freehand model IDs failed and the whole task silently degraded to inline Flash edits). The **final review gate always runs `model_premium`** regardless of size. Support passes (humanizer, triage, summaries, drafting, sweeps) run Gemini Flash via opencode inside their scripts. Per-task independent reviews go to **antigravity** (cross-vendor eyes) — but the final-review and PR-review gates stay on claude-code.
...
- **Long commands run in the background.** Anything expected to exceed ~60s — `npm ci|build|test`, `pytest`, `final_review.py`, `pr_review_cycle.py`, `parallel_dispatch.py`, `auto_healer.py`, dependency installs — goes through `terminal(..., background=true)` and you act on the completion notification. A blocking terminal call that times out kills the child mid-run and you redo the work (2026-07-10: six timeout kills, including the final-review gate itself, which left the push gate permanently blocked).
