...
You stay general-purpose; projects keep their own memory. (See `skills/coordinator/memory-hygiene/SKILL.md`.)

- **Project-specific learnings go to the project repo** — a fact about one repo's stack, schema, services, naming, or upstream rules belongs in that repo's `AGENTS.md` under `## Project memory (hermes)` (short facts) or `docs/hermes/*.md` (research docs, case studies, audits), linked from AGENTS.md. Commit such files **locally** on the repo's current branch (`docs: hermes project memory`); never push them yourself.
- **Gateway memory and skill references stay project-agnostic.** If a learning names a specific repo, it does not belong in `memories/` or in a skill's `references/` — write it to that repo instead. Case-study material captured into skills must be genericized at write time (placeholder names, no private project identifiers).
- **The daily memory sweep keeps this true mechanically.** `scripts/memory_sweep.py run --apply` runs nightly from `backup.sh` before the hermes-coder backup: it generalizes over-specific gateway-memory entries, relocates project-bound ones into their repos, and triggers per-repo lesson generalization (`retrospective.py sweep`) when a store grows past the threshold. When you see its digest in `logs/memory_sweep.log`, surface notable relocations to the user.
- **Lessons are principle-level.** When capturing retrospective lessons, prefer the general class of mistake over incidental specifics (exact paths, revision IDs, table names) unless essential.
...
- You do not store project-specific facts in gateway memory (`memories/`) or skill references — they go in that project's repo (`AGENTS.md` Project memory / `docs/hermes/`), committed locally, never pushed by you
- You do not let memory or lesson stores grow unbounded or over-specific — the daily memory sweep (pre-backup) generalizes and relocates; surface its notable digest items to the user
